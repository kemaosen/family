...
import ImageCard from '../../components/gallery/ImageCard.js'
import Controller from '../../components/gallery/controller'
import '../../components/gallery/gallery.less'
// 获取图片数组相关信息，增加 URL
let ImgInfos = ImgsData.map((img) => {
  return Object.assign({},
    img,
    { url: require(`src/view/pengpeng/image/${ img.filename }`) }
  )
})
...
class Guaer extends Component {
  ...
  reverseFigure(index) {
    return function () {
      ...
      let targetStatus = figureArrangeArr[index].isReverse
      figureArrangeArr[index].isReverse = !targetStatus
      this.setState({
  ...
  render() {
    ...
    ImgInfos.forEach(function (imgInfo, index) {
      ...
      imgFigures.push(<ImageCard data={ imgInfo } key={ index } id={ 'figure' + index }
